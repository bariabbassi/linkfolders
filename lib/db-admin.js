...
export async function getProfile(id) {
  ...
  if (item.type === 'profile') return { profile: item };
}

export async function getProfileUsername(username) {
  ...
  if (profiles.length === 1 && profiles[0].type === 'profile')
...
export async function getRoot(id) {
  ...
  if (item.type === 'root') return { root: item };
}
